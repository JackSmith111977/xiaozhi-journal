#!/usr/bin/env python3
"""
First Breath — Deterministic sanctum scaffolding for agent-dev-gated.
...
Example:
    uv run scripts/init-sanctum.py /Users/me/myproject /path/to/agent-dev-gated
"""
...
def generate_capabilities_md(capabilities: list[dict], evolvable: bool) -> str:
    ...
    if evolvable:
        lines.extend([
            ...
            "## Learned",
            "",
            "_Capabilities added by the owner over time._",
            "",
    ...
    lines.extend([
        ...
        "- WebSearch, WebFetch (联网调研)",
        "",
        "### User-Provided Tools",
        "",
        "_Document any MCP servers or services here._",
    ])
...
def main():
    ...
    # Load config from bmm/config.yaml
    bmm_config_path = bmad_dir / "bmm" / "config.yaml"
    config = parse_yaml_config(bmm_config_path)
    ...
    # Copy reference files (capabilities) into sanctum
